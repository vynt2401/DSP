...
function [hn_HP] = a0_CaoQua(omega_c, N)
    
    n = -(N-1)/2:(N-1)/2;
    hn_HP = zeros(1, length(n)); % Khởi tạo đáp ứng xung

    % Thiết kế đáp ứng xung
    for i = 1:length(n)
        if n(i) == 0
            hn_HP(i) = 1 - omega_c / pi; % Trường hợp n = 0
        else
            hn_HP(i) = -sin(omega_c * n(i)) / (pi * n(i)); % Trường hợp n ≠ 0
        end
    end
end
